...
//			tuio.addListener(TuioFiducialDispatcher.init(stage,1000));
		...
		private function timerHandler(evt:TimerEvent):void{
			...
			for (var f_id1:* in PropObjectDict) {
				tempArray.push(0);
			}
			
			// alle deaktivieren
			for (var f_id2:* in PropObjectDict) {
				var prop1:PropObject = PropObject(PropObjectDict[f_id2]); 
				prop1.isActiv = false;
			}
			
		
			// die lebenden wieder aktivieren
			for (var f_id3:* in PropObjectDict) {
				var prop2:PropObject = PropObject(PropObjectDict[f_id3]); 
				var s_id:* = prop2.s_id;
			...
			for (var f_id4:* in PropObjectDict) {
				//trace(f_id+"----------")
				var prop3:PropObject = PropObject(PropObjectDict[f_id4]);
				if(prop3.isActiv){
					postAliveArray.push(f_id4);
				}else{
			...
			for (var i:*=0; i < postAliveArray.length; i++){
				if( (preAliveArray[i] != postAliveArray[i]) && (postAliveArray[i] == 0)){
					var f_id5:* = preAliveArray[i];
					var prop4:PropObject = PropObject(PropObjectDict[f_id5]);
					prop4.dispatchEvent(new PropEvent(PropEvent.REMOVE_PROP, prop4.s_id, prop4.f_id));
				}
			}
		}
