package org.tuio.connectors {
	...
	public class TCPConnector implements IOSCConnector {
		 ...
		 */
		public function TCPConnector(host:String = "127.0.0.1", port:int = 3333, flosc:Boolean=false) {
			...
			this.connection = new OSCSocket(flosc);
			if (flosc)
				this.connection.addEventListener(OSCEvent.OSC_DATA, receiveFloscData);
			else
		...
		public function receiveFloscData(e:OSCEvent):void {
			if (!e.floscData) return;
			var oscMsg:OSCMessage = new OSCMessage();
			oscMsg.address = "/tuio/2Dcur";
			for each(var arg:* in e.floscData){
				oscMsg.addArgument(arg.type, arg.value);
			}
			for each(var l:IOSCConnectorListener in this.listeners)
				l.acceptOSCPacket(oscMsg);
		}
